...
public class ShareEnrollServlet extends HttpServlet {
	...
	private ShareService shareSerivce = new ShareService();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			String saveDirectory = getServletContext().getRealPath("/upload/share"); // 웹루트디렉토리(src/main/webapp)부터 탐색
	...
	//		FileRenamePolicy policy = new DefaultFileRenamePolicy(); // 중복파일이 있는 경우, abc1.txt, abc2.txt, ...
			...
			// 2. 업무로직 
			int result = shareSerivce.insertShare(share);
